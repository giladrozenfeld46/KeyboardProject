...
static int mem_fd = -1; // Fixed: Added missing mem_fd declaration
...
void smi_manager_write_sequence(const uint8_t* gpio8_seq, const uint8_t* gpio9_seq, size_t length) {
    ...
    struct DmaControlBlock* tx_cbs = (struct DmaControlBlock*)tx_cb_buf.virtual_addr;
    tx_cbs[0].txfr_len = length * sizeof(uint32_t); // Fixed: Changed from length to txfr_len
...
void smi_manager_cleanup() {
    ...
    // Fixed: Updated to new channel pointers
    ...
    // Fixed: mem_fd is now declared globally
    if (mem_fd >= 0) {
        close(mem_fd);
    }
    
    // Fixed: Cleanup logic updated for separated RX/TX buffers
    for (int i = 0; i < NUM_RX_BUFFERS; i++) {
